...
module.exports = function ({plugins, config} = {}) {
  ...
  if (plugins && plugins.policies && plugins.policies.length) {
    plugins.policies.forEach(p => policies.register(p));
  }
  config = config || require('../config');

  config.on('gatewayConfigChange', config => {
    eventBus.emit('hot-reload', {type: 'gateway', config});
  });

  let { httpServer, httpsServer } = bootstrap({plugins, config});
...
function bootstrap ({plugins, config} = {}) {
  ...
  config.gatewayConfig.policies && config.gatewayConfig.policies.forEach(policyName => {
    let policy = policies.resolve(policyName);
    if (policy.routes) {
      policy.routes(app, config);
    }
  });

  if (plugins && plugins.gatewayExtensions && plugins.gatewayExtensions.length) {
    plugins.gatewayExtensions.forEach(ext => ext(app));
  }

  let conditionEngine = conditions.init();
  if (plugins && plugins.conditions && plugins.conditions.length) {
    plugins.conditions.forEach(cond => conditionEngine.register(cond));
  }
